...
- **Última atualização:** 2026-07-31 (entrou o **DRG-Atende24h**, no ar; e a trava de publicação que confere app x landing).
- **🌐 NO AR:** https://drsystems.dev.br (domínio próprio + HTTPS; repo `zett-romao/drg-marketing`, GitHub Pages/main).
- **20 produtos** visíveis na família (Kronos, BuscaNFe, Check, Sign, Jurídico, Hidro, **Guard**, **Atende24h**, Fit, Rently, Garantidora, Liftalk, Sind.ia, Condo, **Access**, **Charge**, **Energy**, **Sense**, **Pulse**, **View**). **Open** e **AI** ficam `visivel:false` no `site.json` (fora da home, reativáveis no `/admin`): Open foi aposentado pelo Access; **AI é motor interno da suíte, sem card próprio** (decisão do dono).
...
4. **Fonte única de dados** (ampliada em 2026-07-25): **TUDO** de cada produto vem de `data/site.json` — card, planos **E a copy da página** (`h1`, `sub`, `dores`, `sol`, `rec`, `passos`, `descPagina` de SEO etc.). O gerador não tem mais lista hardcoded. Home e páginas de produto renderizam via JS com **cache-bust** (`?t=Date.now()`). Nunca hardcodar card/plano/copy no HTML. Status por produto: `ativo` (No ar) ou `embreve` (lista de espera) — um toggle no `/admin` muda home e página juntas.
...
6. **Páginas de produto** geradas por `tools/gen-produtos.mjs`, que agora **lê `data/site.json`** (fonte única) — rodar após mexer no template ou na copy. O gerador também reescreve a **lista do `<noscript>`** e o **contador de produtos** da home a partir do `site.json` (bloco entre `<!-- PRODUTOS:INICIO -->` e `<!-- PRODUTOS:FIM -->` — não editar à mão). **Produto novo em 1 comando:** `node tools/novo-produto.mjs <key> "Nome" "🙂" "Categoria" [ativo|embreve] [https://app...]` — adiciona o esqueleto no `site.json`, gera a página e imprime o passo do Drive. Prévia local por `tools/serve.mjs` (`file://` bloqueia o fetch do JSON).
   - **🔒 Trava de publicação (lock 2026-07-31):** `tools/auditar-produtos.mjs` cruza o `site.json` com **duas fontes** e acusa **app sem card**, **card sem página gerada** e **copy ainda com "TODO"**. As duas fontes são de propósito: **(1) os repos da conta no GitHub** (`gh repo list`, dono lido do remote `origin`) — vale em qualquer máquina; **(2) as pastas `DRG-*` do disco** — pega o app que ainda nem tem repo. Só o disco não bastava: **o Atende24h nasceu no MacBook**, então uma auditoria rodando no PC não veria pasta nenhuma e diria "em dia". Sem rede ou sem `gh`, ela avisa e segue só com o disco; `--sem-github` (ou `DRG_SEM_GITHUB=1`) pula a consulta de propósito. Roda no **hook `pre-push`** (`.githooks/pre-push`) e **barra o push** com pendência; `CONFERIR APPS.bat` (Windows) e `conferir-apps.command` (macOS) rodam na hora. **A trava se instala sozinha:** `tools/_hooks.mjs` acerta o `core.hooksPath` na primeira vez que você roda a auditoria, o gerador ou a prévia — não precisa decorar comando. Emergência: `git push --no-verify`. O que **não** é produto de venda (`marketing`, `platform`, pastas aposentadas) fica declarado em `NAO_E_PRODUTO`, por **key** — vale para repo e para pasta. Nome de repo que não bate com a key do produto (`drg-sistema`→Kronos, `drg-sindico`→Sind.ia) vai em `APELIDOS`.
   - **Raízes da varredura em disco (Windows e macOS):** a pasta-mãe deste repo (os `DRG-*` são irmãos do `DRG-Marketing`), mais `C:\Projetos`, `G:\Meu Drive`, `~/Projetos` e `~/Documents/Projetos`. Layout diferente disso: `DRG_RAIZES="/caminho/um:/caminho/dois"` (`;` separa no Windows) — quando definida, **substitui** a lista padrão. Se **nenhuma** das duas fontes responder, a auditoria **avisa que a metade "app sem card" não rodou** em vez de dizer "em dia", e **não** barra o push. Motivo: o **Atende24h subiu em 30/07 e não tinha card, página nem planos aqui** — o `site.json` já era fonte única, mas nada conferia se nascia app novo, então o produto ia ficar fora da vitrine por tempo indeterminado.
7. **🔒 Rodapé-assinatura DR Systems (marca-mãe) — padrão da FAMÍLIA, em TODOS os apps e landings** (lock 2026-07-09): 3 linhas centralizadas, **grudadas no fundo** (sticky footer: body flex column + rodapé `margin-top:auto`) — nunca flutua no meio:
   ...
   - Rollout feito: Marketing (todas as landings), Sind.ia, Check, Sign, Jurídico, Hidro, BuscaNFe, Fit. **Pendente: Condo e Kronos.**
