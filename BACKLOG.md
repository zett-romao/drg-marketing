...
6. **Páginas de produto** geradas por `tools/gen-produtos.mjs`, que agora **lê `data/site.json`** (fonte única) — rodar após mexer no template ou na copy. O gerador também reescreve a **lista do `<noscript>`** e o **contador de produtos** da home a partir do `site.json` (bloco entre `<!-- PRODUTOS:INICIO -->` e `<!-- PRODUTOS:FIM -->` — não editar à mão). **Produto novo em 1 comando:** `node tools/novo-produto.mjs <key> "Nome" "🙂" "Categoria" [ativo|embreve] [https://app...]` — adiciona o esqueleto no `site.json`, gera a página e imprime o passo do Drive. Prévia local por `tools/serve.mjs` (`file://` bloqueia o fetch do JSON).
   - **🔒 Trava de publicação (lock 2026-07-31):** `tools/auditar-produtos.mjs` varre as pastas `DRG-*` (ver raízes abaixo), cruza com o `site.json` e acusa **app sem card**, **card sem página gerada** e **copy ainda com "TODO"**. Roda no **hook `pre-push`** (`.githooks/pre-push`) e **barra o push** com pendência; `CONFERIR APPS.bat` (Windows) e `conferir-apps.command` (macOS) rodam na hora. **A trava se instala sozinha:** `tools/_hooks.mjs` acerta o `core.hooksPath` na primeira vez que você roda a auditoria, o gerador ou a prévia — não precisa decorar comando. Emergência: `git push --no-verify`. Pasta que **não** é produto de venda (DRG-Platform, DRG-Marketing) fica declarada em `NAO_E_PRODUTO`, dentro do próprio script.
   - **Raízes da varredura (serve em Windows e macOS):** a pasta-mãe deste repo (os `DRG-*` são irmãos do `DRG-Marketing`), mais `C:\Projetos`, `G:\Meu Drive`, `~/Projetos` e `~/Documents/Projetos`. Layout diferente disso: `DRG_RAIZES="/caminho/um:/caminho/dois"` (`;` separa no Windows) — quando definida, **substitui** a lista padrão. Se não achar nenhuma pasta `DRG-*` (máquina nova, só este repo clonado), a auditoria **avisa que a metade "app sem card" não rodou** em vez de dizer "em dia", e **não** barra o push. Motivo: o **Atende24h subiu em 30/07 e não tinha card, página nem planos aqui** — o `site.json` já era fonte única, mas nada conferia se nascia app novo, então o produto ia ficar fora da vitrine por tempo indeterminado.
